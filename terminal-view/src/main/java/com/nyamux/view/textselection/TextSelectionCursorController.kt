...
import com.nyamux.terminal.WcWidth
import com.nyamux.view.R
import com.nyamux.view.TerminalView

class TextSelectionCursorController(
    @JvmField internal val terminalView: TerminalView
) : CursorController {

    @JvmField internal val mStartHandle: TextSelectionHandleView = TextSelectionHandleView(terminalView, this, TextSelectionHandleView.LEFT)
    @JvmField internal val mEndHandle: TextSelectionHandleView = TextSelectionHandleView(terminalView, this, TextSelectionHandleView.RIGHT)
    @JvmField internal var mStoredSelectedText: String? = null
    @JvmField internal var mIsSelectingText: Boolean = false
    @JvmField internal var mShowStartTime: Long = System.currentTimeMillis()

    @JvmField internal val mHandleHeight: Int
    @JvmField internal var mSelX1: Int = -1
    @JvmField internal var mSelX2: Int = -1
    @JvmField internal var mSelY1: Int = -1
    @JvmField internal var mSelY2: Int = -1

    @JvmField internal var mActionMode: ActionMode? = null
    @JvmField val ACTION_COPY: Int = 1
    @JvmField val ACTION_PASTE: Int = 2
    @JvmField val ACTION_MORE: Int = 3
    ...
    fun setActionModeCallBacks() {
        ...
        mActionMode = terminalView.startActionMode(object : ActionMode.Callback2() {
            ...
            override fun onGetContentRect(mode: ActionMode?, view: View?, outRect: Rect?) {
                ...
                var x1 = Math.round(mSelX1 * terminalView.mRenderer!!.mFontWidth)
                var x2 = Math.round(mSelX2 * terminalView.mRenderer!!.mFontWidth)
                val y1 = Math.round((mSelY1 - 1 - terminalView.getTopRow()) * terminalView.mRenderer!!.mFontLineSpacing.toFloat())
                val y2 = Math.round((mSelY2 + 1 - terminalView.getTopRow()) * terminalView.mRenderer!!.mFontLineSpacing.toFloat())
    ...
    override fun updatePosition(handle: TextSelectionHandleView?, x: Int, y: Int) {
        val screen = terminalView.mEmulator!!.screen
        val scrollRows = screen.getActiveRows() - terminalView.mEmulator!!.mRows
     ...
     */
    fun isSelectionStartDragged(): Boolean {
     ...
     */
    fun isSelectionEndDragged(): Boolean {
