...
import com.nyamux.shared.net.uri.UriScheme
...
class TermuxOpenReceiver : BroadcastReceiver() {
    ...
    class ContentProvider : android.content.ContentProvider() {
        ...
        override fun query(
            ...
            val row = arrayOfNulls<Any>(resolvedProjection.size)
            for (i in resolvedProjection.indices) {
                val column = resolvedProjection[i]
        ...
        override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
            ...
            try {
                ...
                Logger.logDebug(
                    LOG_TAG,
                    "Open file request received from $callingPackageName for \"$path\" with mode \"$mode\""
                )
