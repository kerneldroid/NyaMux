 ...
 * and {@link com.nyamux.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 ...
 */
class TermuxActivityRootView : LinearLayout, ViewTreeObserver.OnGlobalLayoutListener {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val currentMarginBottom = marginBottom
        if (currentMarginBottom != null) {
            if (ROOT_VIEW_LOGGING_ENABLED) {
                Logger.logVerbose(LOG_TAG, "onMeasure: Setting bottom margin to $currentMarginBottom")
            }
            val params = layoutParams as ViewGroup.MarginLayoutParams
            params.setMargins(0, 0, 0, currentMarginBottom)
            layoutParams = params
            marginBottom = null
            requestLayout()
        }
    ...
    override fun onGlobalLayout() {
        ...
        val windowAndViewRects = ViewUtils.getWindowAndViewRects(bottomSpaceView, mStatusBarHeight) ?: return
        ...
        if (root_view_logging_enabled) {
            Logger.logVerbose(
                LOG_TAG,
                "windowAvailableRect " + ViewUtils.toRectString(windowAvailableRect) + ", bottomSpaceViewRect " + ViewUtils.toRectString(
                    bottomSpaceViewRect
                )
            )
            Logger.logVerbose(
                LOG_TAG,
                ("windowAvailableRect.bottom " + windowAvailableRect.bottom +
                        ...
                        ", isVisible " + windowAvailableRect.contains(bottomSpaceViewRect) + ", isRectAbove " + ViewUtils.isRectAbove(
                    windowAvailableRect,
                    bottomSpaceViewRect
                ) +
