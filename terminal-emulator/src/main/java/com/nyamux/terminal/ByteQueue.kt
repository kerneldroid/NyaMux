...
internal class ByteQueue(size: Int) {
    ...
    fun write(buffer: ByteArray, offset: Int, lengthToWrite: Int): Boolean {
        ...
        var remainingLengthToWrite = lengthToWrite

        if (remainingLengthToWrite + currentOffset > buffer.size) {
            throw IllegalArgumentException("length + offset > buffer.length")
        } else if (remainingLengthToWrite <= 0) {
        ...
        synchronized(this) {
            while (remainingLengthToWrite > 0) {
                ...
                var bytesToWriteBeforeWaiting = min(remainingLengthToWrite, bufferLength - mStoredBytes)
                remainingLengthToWrite -= bytesToWriteBeforeWaiting
