...
class TerminalBuffer(
        ...
        if (newColumns == mColumns && newRows <= mTotalRows) {
            ...
            if (shiftDownOfTopRow > 0 && shiftDownOfTopRow < mScreenRows) {
                ...
                for (i in mScreenRows - 1 downTo 1) {
                    ...
                    val rowLine = mLines[r]
                    if (rowLine == null || rowLine.isBlank()) {
                        if (--shiftDownOfTopRow == 0) break
                    }
                }
            } else if (shiftDownOfTopRow < 0) {
                // Negative shift down = expanding. Only move screen up if there is transcript to show:
                val actualShift = shiftDownOfTopRow.coerceAtLeast(-mActiveTranscriptRows)
            ...
            mActiveTranscriptRows = if (altScreen) 0 else 0.coerceAtLeast(mActiveTranscriptRows + shiftDownOfTopRow)
            cursor[1] -= shiftDownOfTopRow
            mScreenRows = newRows
        } else {
            // Copy away old state and update new:
            val oldLines = mLines
            mLines = Array<TerminalRow?>(newTotalRows) { TerminalRow(newColumns, currentStyle) }
            ...
            for (externalOldRow in -oldActiveTranscriptRows until oldScreenRows) {
                ...
                if (oldLine == null || (!newCursorPlaced && cursorAtThisRow).not() && oldLine.isBlank()) {
                    skippedBlankLines++
                    continue
    ...
    fun blockSet(sx: Int, sy: Int, w: Int, h: Int, valChar: Int, style: Long) {
        if (sx < 0 || sx + w > mColumns || sy < 0 || sy + h > mScreenRows) {
            throw IllegalArgumentException(
                "Illegal arguments! blockSet($sx, $sy, $w, $h, $valChar, $mColumns, $mScreenRows)"
            )
        }
        for (y in 0 until h) {
            for (x in 0 until w) {
    ...
    fun clearTranscript() {
        if (mScreenFirstRow < mActiveTranscriptRows) {
            java.util.Arrays.fill(mLines, mTotalRows + mScreenFirstRow - mActiveTranscriptRows, mTotalRows, null)
            java.util.Arrays.fill(mLines, 0, mScreenFirstRow, null)
        } else {
            java.util.Arrays.fill(mLines, mScreenFirstRow - mActiveTranscriptRows, mScreenFirstRow, null)
        }
        mActiveTranscriptRows = 0
    }
}
