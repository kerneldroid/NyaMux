package com.nyamux.shared.android
...
object AndroidUtils {
    ...
    fun getAppInfoMarkdownString(context: Context, packageName: String): String? {
        ...
        val seInfo = PackageUtils.getApplicationInfoSeInfoForPackage(applicationInfo)
        val seInfoUser = PackageUtils.getApplicationInfoSeInfoUserForPackage(applicationInfo)
        appendPropertyToMarkdown(
            markdownString, "SE_INFO", "${seInfo ?: ""}${if (DataUtils.isNullOrEmpty(seInfoUser)) "" else seInfoUser}"
        )
    ...
    @JvmStatic
    @JvmOverloads
    fun getDeviceInfoMarkdownString(context: Context, addPhantomProcessesInfo: Boolean = false): String {
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.R) {
            ...
            if (maxPhantomProcesses != null) {
                ...
                appendLiteralPropertyToMarkdown(
                    markdownString,
