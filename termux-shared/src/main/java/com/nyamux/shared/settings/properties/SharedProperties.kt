package com.nyamux.shared.settings.properties
...
import com.nyamux.shared.file.filesystem.FileType
...
class SharedProperties(
     ...
     */
    private var mMap = HashMap<String, Any?>()
    ...
    companion object {
        ...
        fun getPropertiesFromFile(
            ...
            try {
                FileInputStream(propertiesFile).use { `in` ->
                    Logger.logVerbose(LOG_TAG, "Loading properties from \"" + propertiesFile.absolutePath + "\" file")
                    properties.load(InputStreamReader(`in`, StandardCharsets.UTF_8))
                }
            } catch (e: Exception) {
                if (context != null) {
                    Toast.makeText(
        ...
        fun getPropertiesFileFromList(propertiesFilePaths: List<String>?, logTag: String): File? {
            ...
            for (propertiesFilePath in propertiesFilePaths) {
                ...
                if (fileType == FileType.REGULAR) {
                    ...
                    Logger.logWarn(
                        logTag,
                        "Ignoring properties file at \"$propertiesFilePath\" of type: \"" + fileType.getName() + "\""
                    )
                }
            }
         ...
         *
         * @param map The [HashMap] object to add value to.
         ...
         */
        @JvmStatic
        fun getBooleanValueForStringValue(
            key: String?,
            value: String?,
            def: Boolean,
            logErrorOnInvalidValue: Boolean,
         ...
         */
        @JvmStatic
        fun getInvertedBooleanValueForStringValue(
            key: String?,
            value: String?,
            def: Boolean,
            logErrorOnInvalidValue: Boolean,
            ...
            if (outputValue == null) {
                ...
                if (logErrorOnInvalidValue && inputValue != null) {
                    if (key != null) {
                        Logger.logError(
                            logTag,
                            "The value \"" + inputValue + "\" for the key \"" + key + "\" is invalid. Using default value \"" + defaultInputValue + "\" instead."
                        )
                    } else {
                        Logger.logError(
                            logTag,
                            "The value \"" + inputValue + "\" is invalid. Using default value \"" + defaultInputValue + "\" instead."
                        )
                    }
                }
            ...
            if (value < min || value > max) {
                if (logErrorOnInvalidValue && (!ignoreErrorIfValueZero || value != 0)) {
                    if (key != null) {
                        Logger.logError(
                            logTag,
                            "The value \"" + value + "\" for the key \"" + key + "\" is not within the range " + min + "-" + max + " (inclusive). Using default value \"" + def + "\" instead."
                        )
                    } else {
                        Logger.logError(
                            logTag,
                            "The value \"" + value + "\" is not within the range " + min + "-" + max + " (inclusive). Using default value \"" + def + "\" instead."
                        )
                    }
                }
                return def
